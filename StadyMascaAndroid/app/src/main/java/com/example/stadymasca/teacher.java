    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()){
            ...
            case R.id.nav_profil:
                getSupportFragmentManager().beginTransaction().replace(R.id.btn_container,new Profil_Prof()).commit();

                break;

            case R.id.nav_pub:
                Intent intent = new Intent(teacher.this,publication.class);
                startActivity(intent);
                finish();
                break;
            case R.id.nav_logout:
                sessionManager.logout3();
                break;
